...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com" xmlns:hl7="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="isc hl7 xsi exsl">
	...
	<xsl:template match="*" mode="HealthFunds">
		...
		<xsl:apply-templates select=".//hl7:act[hl7:templateId/@root=$payersEntryCoverageTemplateId and not(.//@negationInd='true')]" mode="HealthFund"/>
	</xsl:template>
	...
	<xsl:template match="*" mode="PayersSection">
		<xsl:if test="$memberEnrollmentImportMode = '1'">
			<xsl:apply-templates select="$sectionRootPath[hl7:templateId/@root=$payersSectionTemplateId]" mode="PayersSectionEntries"/>
		</xsl:if>
	</xsl:template>
		...
		<xsl:if test="$isNoDataSection='0' or $documentActionCode='XFRM'">
			<MemberEnrollments>
				...
				<xsl:if test="$isNoDataSection='0'">
					<xsl:apply-templates select=".//hl7:act[hl7:templateId/@root=$payersEntryCoverageTemplateId and not(.//@negationInd='true')]" mode="MemberEnrollment"/>
				</xsl:if>
			</MemberEnrollments>
		</xsl:if>
	</xsl:template>
  ...
  <!--
	...
	<xsl:template match="hl7:section" mode="IsNoDataSection-Payers-MemberEnrollments">
		<xsl:choose>
			...
			<xsl:when test="count(.//hl7:act[hl7:templateId/@root=$payersEntryCoverageTemplateId and not(.//@negationInd='true')]) = 0">1</xsl:when>
