...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com" xmlns:hl7="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="isc hl7 sdtc xsi exsl">
	...
	<xsl:template match="*" mode="DefaultEncounter">
		...
		<Encounters>
			<Encounter>
				...
				<xsl:variable name="isNoDataSectionHF"><xsl:apply-templates select="$sectionRootPath[hl7:templateId/@root=$ccda-PayersSection]" mode="IsNoDataSection-Payers"/></xsl:variable>
				<xsl:variable name="sectionEntriesHF" select="$sectionRootPath[hl7:templateId/@root=$ccda-PayersSection]/hl7:entry"/>
				
				<!-- ActionCode is not supported for HealthFund, causes parse error in SDA3. -->
				
				<xsl:choose>
					<xsl:when test="$sectionEntriesHF and $isNoDataSectionHF='0' and $healthFundImportMode = '1'">
						<HealthFunds>
							<xsl:apply-templates select="$sectionEntriesHF" mode="HealthFunds"/>
						</HealthFunds>
					</xsl:when>
				</xsl:choose>
				...
				<!--
					Because HealthShare supports both Instructions and Hospital
					Discharge Instructions as text-only sections, a no-data
					section check does not apply to these sections.
				-->
				<xsl:variable name="sectionNarrativeHDInstructions" select="$sectionRootPath[hl7:templateId/@root=$ccda-HospitalDischargeInstructionsSection]"/>
				<xsl:variable name="sectionNarrativeInstructions" select="$sectionRootPath[hl7:templateId/@root=$ccda-InstructionsSection]"/>
    			...
    			<!--
					...
					However, to not lose data, we will import Discharge Instructions and/or Instructions as long as the document contains a encounter.
	 			-->				
				<xsl:choose>
					<xsl:when test="$encounterCount>=1 and ($sectionNarrativeHDInstructions or $sectionNarrativeInstructions)">
						<RecommendationsProvided>
							<xsl:apply-templates select="$sectionNarrativeHDInstructions" mode="DischargeInstructionsSection-Narrative"/>
							<xsl:apply-templates select="$sectionNarrativeInstructions" mode="DischargeInstructionsSection-Narrative"/>
						</RecommendationsProvided>
					</xsl:when>
				</xsl:choose>
	...
	<xsl:template match="hl7:encounter" mode="OverriddenEncounter">
		<!--
			$encounterIDTemp is used as an intermediate so that
			$encounterID can be set such that "000nnn" does NOT
			match "nnn" when comparing encounter numbers.
		...
		<Encounters>
			<Encounter>
				...
				<!--
					Hospital Discharge Instructions and Instructions are supported
					only as narrative-only sections (no entries).  There is no
					mechanism in that section to tie instructions to encounters.
					Also, those sections should be from a single-encounter document.
					Therefore import Discharge Instructions or Instructions only
					when the document contains a single encounter.
				-->
				<xsl:choose>
					<xsl:when test="$encounterCount=1 and ($sectionNarrativeHDInstructions or $sectionNarrativeInstructions)">
						<RecommendationsProvided>
	...
	<xsl:template match="*" mode="DuplicatedEncounter">
		...
		<xsl:variable name="overriddenEncounterRootPath" select="$sectionRootPath[hl7:templateId/@root=$ccda-EncountersSectionEntriesOptional or hl7:templateId/@root=$ccda-EncountersSectionEntriesRequired]/hl7:entry/hl7:encounter[(hl7:id/@extension=$duplicatedEncounterID and hl7:id/@root=$duplicatedEncounterOrg) or (not(hl7:id/@extension) and hl7:id/@root=$duplicatedEncounterID)]"/>
	...
	<xsl:template match="*" mode="DefaultEncounterDetail">
		...
		<xsl:if test="(count(hl7:encounterParticipant[@typeCode]) > 0) or (count(hl7:performer[@typeCode]) > 0)" >
			<Participants>
				<xsl:apply-templates select="." mode="EncounterParticipants"/>
			</Participants>
		</xsl:if>
		...
		<xsl:choose>
			<xsl:when test="hl7:location">
				...
				<Locations>
					<xsl:apply-templates select="hl7:location" mode="HealthCareFacilityLocation"/>
				</Locations>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="hl7:encounter" mode="OverriddenEncounterDetail">
		<!--
		...
		<!--
			...
			Target: HS.SDA3.Encounter EnteredAt
			Target: /Container/Encounters/Encounter/EnteredAt
			Source: /ClinicalDocument/component/structuredBody/component/section[templateId/@root='2.16.840.1.113883.10.20.22.2.22.1']/entry/encounter/informant
			StructuredMappingRef: EnteredAt
		-->
		...
		<xsl:if test="(count(hl7:encounterParticipant[@typeCode]) > 0) or (count(hl7:performer[@typeCode]) > 0)" >
			<Participants>
				<xsl:apply-templates select="." mode="EncounterParticipants"/>
				<xsl:apply-templates select="." mode="EncounterParticipants-informants"/>
			</Participants>
		</xsl:if>
	...
	<xsl:template match="*" mode="DuplicatedEncounterDetail">
		...
		<!-- EnteredBy - will get from $defaultAuthorRootPath if not found in Encounters section -->
		<xsl:apply-templates select="$overriddenEncounterRootPath" mode="EnteredBy"/>
				
		<!-- HS.SDA3.Encounter EnteredAt -->		
		<!-- EnteredAt - will get from $defaultInformantRootPath if not found in Encounters section -->
		...
		-->
        <xsl:apply-templates select="." mode="EncounterParticipantsDuplicate">
          <xsl:with-param name="overriddenEncounterRootPath" select="$overriddenEncounterRootPath"/>
          <xsl:with-param name="encompassingEncounterRootPath" select="$encompassingEncounterRootPath"/>
        </xsl:apply-templates> 
		...
		<xsl:choose>
			<xsl:when test="$encompassingEncounterRootPath/hl7:location">
				...
				<Locations>
					<xsl:apply-templates select="$encompassingEncounterRootPath/hl7:location" mode="HealthCareFacilityLocation"/>
				</Locations>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="hl7:code" mode="EncounterCodedType">
  	...
  	<xsl:template match="*" mode="EncounterParticipantsDuplicate">
    	<xsl:param name="overriddenEncounterRootPath"/>
		<xsl:param name="encompassingEncounterRootPath"/>
    	<!--
		...
		<xsl:if test="(count(hl7:encounterParticipant[@typeCode]) > 0) or (count(hl7:performer[@typeCode]) > 0)" >
			<Participants>
				<xsl:apply-templates select="." mode="EncounterParticipants"/>
				<xsl:apply-templates
		  			select="$overriddenEncounterRootPath/hl7:performer[not(string-length(hl7:assignedEntity/hl7:id/@root)
		    				and contains($encompassingEncounterParticipantIds, concat('|', hl7:assignedEntity/hl7:id/@root, '/', hl7:assignedEntity/hl7:id/@extension, '|')))]"
		  			 mode="EncounterParticipants"/>
			</Participants>
  		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="*" mode="encounter-HealthCareFacility">
		...
		<xsl:choose>
			<xsl:when test="hl7:participant[@typeCode='LOC']/hl7:participantRole[@classCode='SDLOC']">
				...
				<xsl:if test="count(hl7:participant[@typeCode='LOC']/hl7:participantRole[@classCode='SDLOC' and hl7:templateId/@root=$ccda-ServiceDeliveryLocation]) > 0">
					<Locations>
						<xsl:apply-templates select="hl7:participant[@typeCode='LOC']/hl7:participantRole[@classCode='SDLOC' and hl7:templateId/@root=$ccda-ServiceDeliveryLocation]" mode="ServiceDeliveryLocation"/>
					</Locations>
				</xsl:if>
			</xsl:when>
			<!--
