...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com"
  ...
  <!-- For repOrgconcatIdRootAndNumericExt the value in ImportProfile.xsl must be '1' or missing altogether in order to enable concat. -->
	...
	<!-- 
		Global variable for structured section narrative import
	-->
	<xsl:variable name="narrativeImportStructured" select="exsl:node-set($narrativeImportConfiguration)"/>
	<xsl:variable name="narrativeImportWrapWidth">
		<xsl:choose>
			<xsl:when test="string-length($narrativeImportStructured/wrapWidth/text())">
				<xsl:value-of select="$narrativeImportStructured/wrapWidth/text()"/>
			</xsl:when>
	...
	<!-- 
		Global variables for payer section handling
		By default as HealthFunds (not MemberEnrollments) for backward compatibility 
	-->
	<xsl:variable name="healthFundImportMode">
		<xsl:choose>
			<xsl:when test="'0' = exsl:node-set($encountersImportConfiguration)/healthFundImportMode/text()">0</xsl:when>
			<xsl:otherwise>1</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="memberEnrollmentImportMode">
		<xsl:choose>
			<xsl:when test="'1' = exsl:node-set($payersImportConfiguration)/memberEnrollmentImportMode/text()">1</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>

	<!-- 
		Global variables for gender identity handling
		By default as SocialHistory (not Patient:GenderIdentity) for backward compatibility 
	-->
	<xsl:variable name="patientGenderIdentityImportMode">
		<xsl:choose>
			<xsl:when test="'1' = exsl:node-set($socialHistoryImportConfiguration)/patientGenderIdentityImportMode/text()">1</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="socialHistoryGenderIdentityImportMode">
		<xsl:choose>
			<xsl:when test="'0' = exsl:node-set($socialHistoryImportConfiguration)/socialHistoryGenderIdentityImportMode/text()">0</xsl:when>
			<xsl:otherwise>1</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>

	<!-- 
		Global variables for notes section handling
	-->
	<xsl:variable name="noteIncludeSections" select="exsl:node-set($notesImportConfiguration)/includeSections/text()"/>
	<xsl:variable name="noteExcludeSections" select="exsl:node-set($notesImportConfiguration)/excludeSections/text()"/>
	<xsl:variable name="noteIncludeNotes" select="exsl:node-set($notesImportConfiguration)/includeNotes/text()"/>
	<xsl:variable name="noteExcludeNotes" select="exsl:node-set($notesImportConfiguration)/excludeNotes/text()"/>
	...
	<xsl:variable name="encompassingEncounterIdConcat">
	  <!-- encompassingEncounter ID, if any -->
	  	<xsl:for-each select="$input/hl7:componentOf/hl7:encompassingEncounter/hl7:id">
			<xsl:choose>
	...
	<xsl:variable name="encounterIDs">
	  ...
	  <xsl:apply-templates select="$input" mode="cV-getEncounterIDs">
			<xsl:with-param name="encompassingEncounterIdConcat" select="$encompassingEncounterIdConcat"/>
		</xsl:apply-templates>
	</xsl:variable>
	...
	<xsl:template match="*" mode="cV-getEncounterIDs">
		<xsl:param name="encompassingEncounterIdConcat"/>
	  <!-- Gather all of the Encounter IDs into a vertical bar-delimited string. -->

		<xsl:if test="string-length($encompassingEncounterIdConcat)"><xsl:value-of select="$encompassingEncounterIdConcat"/></xsl:if>
		...
		<xsl:if test="string-length($encountersSectionEncounterIDs) or string-length($encompassingEncounterIdConcat)">|</xsl:if>
	</xsl:template>
	...
	<xsl:template match="hl7:entry" mode="cV-getEncounterIdFromEntry">
		...
		-->
		<xsl:for-each select="hl7:encounter/hl7:id">
			<xsl:choose>
				<xsl:when test="string-length(@extension)">
					<xsl:value-of select="concat('|',@extension)"/>
				</xsl:when>
