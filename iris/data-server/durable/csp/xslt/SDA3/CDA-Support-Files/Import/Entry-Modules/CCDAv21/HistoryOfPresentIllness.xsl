...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" exclude-result-prefixes="hl7">
	...
	<xsl:template match="hl7:act" mode="eHOPI-IllnessHistory">
		<!-- The mode formerly known as PresentIllness -->
		<IllnessHistory>
			<!--
			...
			-->
			<NoteText><xsl:apply-templates select="hl7:code" mode="fn-TextValue"/></NoteText>
			
			<!-- Custom SDA Data-->
			<xsl:apply-templates select="." mode="eHOPI-ImportCustom-IllnessHistory"/>
		</IllnessHistory>
	</xsl:template>
	...
	-->
	<xsl:template match="*" mode="eHOPI-ImportCustom-IllnessHistory">
	</xsl:template>
