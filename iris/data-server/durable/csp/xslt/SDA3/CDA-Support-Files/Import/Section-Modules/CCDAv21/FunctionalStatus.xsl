...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" exclude-result-prefixes="hl7">
	...
	<xsl:template match="*" mode="sFS-FunctionalStatusSection">
		<xsl:apply-templates select="key('sectionsByRoot',$ccda-FunctionalStatusSection)" mode="sFS-FunctionalStatusSectionEntries"/>
	</xsl:template>
	
	<xsl:template match="hl7:section" mode="sFS-FunctionalStatusSectionEntries">
		<xsl:variable name="isNoDataSection"><xsl:apply-templates select="." mode="sFS-IsNoDataSection-FunctionalStatus"/></xsl:variable>
		<!--
			...
			are not imported.
		-->
		<xsl:variable name="sectionEntries" select="hl7:entry[not(.//@negationInd='true') and ((not(string-length(.//hl7:encounter/hl7:id/@extension)) and not(string-length(.//hl7:encounter/hl7:id/@root)))
			                                                     or contains($encounterIDs,concat('|',.//hl7:encounter/hl7:id/@extension,'|'))
			                                                     or contains($encounterIDs,concat('|',.//hl7:encounter/hl7:id/@root,'|')))
			                                                   and (hl7:observation/hl7:templateId/@root=$ccda-FunctionalStatusProblemObservation or hl7:observation/hl7:templateId/@root=$ccda-CognitiveStatusProblemObservation)]"/>
		
		<xsl:choose>
			<xsl:when test="$sectionEntries and $isNoDataSection='0'">
				<Problems>
					<xsl:apply-templates select="$sectionEntries" mode="sFS-FunctionalStatusEntries"/>
				</Problems>
			</xsl:when>
	...
	<xsl:template match="hl7:entry" mode="sFS-FunctionalStatusEntries">
		...
		<xsl:apply-templates select="hl7:observation" mode="eFS-Problem-FunctionalStatus"/>
	</xsl:template>
	...
	-->
	<xsl:template match="hl7:section" mode="sFS-IsNoDataSection-FunctionalStatus">
		<xsl:choose>
