...
<xsl:stylesheet version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" exclude-result-prefixes="hl7 xsi">
		...
		<xsl:if test="$IsNoDataSection='0' or $documentActionCode='XFRM'">
			<AdvanceDirectives>
				...
				<xsl:if test="$IsNoDataSection='0'">
					<xsl:apply-templates select="$sectionObservations" mode="eAD-AdvanceDirective"/>
				</xsl:if>
			</AdvanceDirectives>
		</xsl:if>
	</xsl:template>
