...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc" exclude-result-prefixes="hl7">
	...
	<xsl:template match="*" mode="eS-Support">
		...
		<xsl:if test="hl7:participant[@typeCode='IND']">
			<SupportContacts>
				...
				<xsl:apply-templates select="hl7:recordTarget/hl7:patientRole/hl7:patient/hl7:guardian" mode="eS-GuardianInfo"/>
			</SupportContacts>
		</xsl:if>

		<xsl:if test="$patientGenderIdentityImportMode = '1'">
			<xsl:apply-templates select="$sectionRootPath[hl7:templateId/@root=$ccda-SocialHistorySection]/hl7:entry/hl7:observation" mode="eS-GenderIdentity"/>
		</xsl:if>
	</xsl:template>

	<xsl:template match="hl7:observation" mode="eS-GenderIdentity">
		<!--
				Field : Patient GenderIdentity
				Target: HS.SDA3.Patient GenderIdentity
				Target: /Container/Patient/GenderIdentity
				Source: /ClinicalDocument/component/structuredBody/component/section[templateId/@root='2.16.840.1.113883.10.20.22.2.17']/entry/observation
		-->
		<xsl:if test="hl7:templateId/@root=$ccda-SocialHistoryObservation and hl7:code/@code='76691-5'">
			<xsl:apply-templates select="hl7:value" mode="fn-CodeTable">
				<xsl:with-param name="hsElementName" select="'GenderIdentity'"/>
			</xsl:apply-templates>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="hl7:participant" mode="eS-Organization-CarePlan">
		<!-- The mode maps participant to Care Plan Organizations -->	
			...
			-->
			<xsl:apply-templates select="hl7:associatedEntity/hl7:scopingOrganization" mode="fn-OrganizationDetail">
			<xsl:with-param name="emitElementName" select="'DocumentOrganization'"/>
			</xsl:apply-templates>
	...
	<xsl:template match="*" mode="eS-GuardianInfo">
		<SupportContact>

			<!--
				Field : Guardian Name
				Target: HS.SDA3.Patient SupportContacts.SupportContact.Name
				Target: /Container/Patient/SupportContacts/SupportContact/Name
				Source: /ClinicalDocument/recordTarget/patientRole/patient/guardian/guardianPerson/name
				StructuredMappingRef: ContactName
			-->
			<xsl:apply-templates select="hl7:guardianPerson/hl7:name" mode="fn-T-pName-ContactName"/>

			<!--
				Field : Guardian Address
				Target: HS.SDA3.Patient SupportContacts.SupportContact.Address
				Target: /Container/Patient/SupportContacts/SupportContact/Address
				Source: /ClinicalDocument/recordTarget/patientRole/patient/guardian/addr
				StructuredMappingRef: Address
			-->
			<xsl:apply-templates select="hl7:addr" mode="fn-T-pName-address"/>

			<!--
				Field : Guardian ContactInfo
				Target: HS.SDA3.Patient SupportContacts.SupportContact.ContactInfo
				Target: /Container/Patient/SupportContacts/SupportContact/ContactInfo
				Source: /ClinicalDocument/recordTarget/patientRole/patient/guardian/telecom
				StructuredMappingRef: ContactInfo
			-->
			...
			<!--
				...
				Target: HS.SDA3.Patient SupportContacts.SupportContact.Relationship
				Target: /Container/Patient/SupportContacts/SupportContact/Relationship
				Source: /ClinicalDocument/recordTarget/patientRole/patient/guardian/code
			-->
			<!--
				Field : Guardian Relationship
				Target: HS.SDA3.Patient SupportContacts.SupportContact.Relationship
				Target: /Container/Patient/SupportContacts/SupportContact/Relationship
				Source: /ClinicalDocument/recordTarget/patientRole/patient/guardian/guardianPerson/sdtc:asPatientRelationship/code
			-->
			<Relationships>
				<Relationship>
					<Code>GUARD</Code>
					<Description>Guardian</Description>
					<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-RoleCode</SDACodingStandard>
				</Relationship>
				<xsl:apply-templates select="hl7:code" mode="fn-CodeTable">
					<xsl:with-param name="hsElementName" select="'Relationship'"/>
				</xsl:apply-templates>
				<xsl:apply-templates select="hl7:guardianPerson/sdtc:asPatientRelationship/hl7:code" mode="fn-CodeTable">
					<xsl:with-param name="hsElementName" select="'Relationship'"/>
				</xsl:apply-templates>
			</Relationships>
