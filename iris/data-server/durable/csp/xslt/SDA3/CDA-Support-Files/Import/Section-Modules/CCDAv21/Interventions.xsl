...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" exclude-result-prefixes="hl7">
	...
	<xsl:template match="*" mode="sIn-Section">
		<xsl:apply-templates select="key('sectionsByRoot',$ccda-InterventionsSection)" mode="sIn-SectionEntries"/>
	</xsl:template>
	
	<xsl:template match="hl7:section" mode="sIn-SectionEntries">
		<xsl:variable name="isNoDataSection"><xsl:apply-templates select="." mode="sIn-IsNoDataSection"/></xsl:variable>
		<xsl:if test="$isNoDataSection='0'">
			<Interventions>
				<xsl:apply-templates select="hl7:entry[not(.//@negationInd='true')]" mode="eIn-InterventionEntry"/>
			</Interventions>
		</xsl:if>
	</xsl:template>
	
	<!-- Determine if the Interventions section is present but has or indicates no data present.
	...
	-->
	<xsl:template match="hl7:section" mode="sIn-IsNoDataSection">
		<xsl:choose>
