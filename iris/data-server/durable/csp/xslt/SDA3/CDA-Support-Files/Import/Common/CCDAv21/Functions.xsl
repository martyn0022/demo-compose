...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com"
  ...
  xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="isc hl7 sdtc xsi exsl">
			...
			<xsl:choose>
				<xsl:when test="not($useFirstTranslation='1')">
					<xsl:choose>
						...
						<xsl:when test="string-length(normalize-space($codingStandard)) and string-length(normalize-space($code))"><xsl:value-of select="isc:evaluate('getRegistryDescription', $codingStandard, $code)"/></xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
			...
			<PriorCodes>
				<xsl:choose>
					<xsl:when test="$useFirstTranslation='1'"><!-- The first one was used already, take the rest -->
						<xsl:apply-templates select="hl7:translation[not(position()=1) and @code]" mode="fn-PriorCode"/>
					</xsl:when>
					<xsl:otherwise><!-- Take them all -->
						<xsl:apply-templates select="hl7:translation[@code]" mode="fn-PriorCode"/>
					</xsl:otherwise>
				</xsl:choose>
			</PriorCodes>
		</xsl:if>
	...
	<xsl:template match="hl7:author | hl7:informant" mode="fn-SendingFacilityFromOrgInfo">
		<xsl:variable name="facilityInformation"><xsl:apply-templates select=".//hl7:representedOrganization | .//hl7:providerOrganizaton" mode="fn-P-FacilityInformation"/></xsl:variable>
	...
	<xsl:template match="*" mode="fn-EnteredAt">
	  <!-- Match could be hl7:recordTarget/patientRole, hl7:author, hl7:participant, hl7:informant, hl7:observation, more... -->
	  <!--
			...
			Source: ./
			StructuredMappingRef: OrganizationDetail
			Note  : If recordTarget/patientRole/providerOrganization is not present then informat is used.
					- Else if informant is not present then author is used.
		...
		<xsl:choose>
			...
			<xsl:when test="$input/hl7:recordTarget/hl7:patientRole/hl7:providerOrganization"><xsl:apply-templates select="$input/hl7:recordTarget/hl7:patientRole" mode="fn-OrganizationDetail"/></xsl:when>
	...
	<xsl:template match="*" mode="fn-EnteringOrganization">
	  ...
	  <!--
			...
			Source: assignedEntity/representedOrganization/name/text()
		-->
		<xsl:variable name="eoTypes">|INF|AUT|ENT|LOC|ORG|PRF|SPRF|</xsl:variable>
		<xsl:choose>
			...
			<xsl:when test="hl7:participant[contains($eoTypes, concat('|',@typeCode,'|'))]">
				<xsl:apply-templates select="hl7:participant[contains($eoTypes, concat('|',@typeCode,'|'))]" mode="fn-HealthCareFacilityDetail">
					<xsl:with-param name="elementName" select="'EnteringOrganization'"/>
				</xsl:apply-templates>
			</xsl:when>			
			<xsl:otherwise>
	...
	<xsl:template match="hl7:informant | hl7:location" mode="fn-HealthCareFacilityDetail">
		...
		<xsl:variable name="locationOIDRoot" select="substring-before(substring-after($locationInformation, 'L4:'), '|')"/>
		<xsl:variable name="locationOIDExt" select="substring-before(substring-after($locationInformation, 'L5:'), '|')"/>

		<!-- Parse facility information -->
		<xsl:variable name="facilityFlags" select="substring-before(substring-after($facilityInformation, 'F0:'), '|')"/>
		<xsl:variable name="facilityOID" select="substring-before(substring-after($facilityInformation, 'F1:'), '|')"/>
		...
		<xsl:variable name="facilityCodeFromIDs" select="substring-before(substring-after($facilityInformation, 'F4:'), '|')"/>
		
		<!-- IDs for OtherIdentifiers-->
		<xsl:variable name="facilityInformationIDs" select="(.//hl7:representedOrganization/hl7:id | .//hl7:serviceProviderOrganization/hl7:id)[@root != $facilityOID or @extension != $facilityCodeFromIDs]"/>
		<xsl:variable name="locationInformationIDs" select="hl7:healthCareFacility/hl7:id[@root != $locationOIDRoot or @extension != $locationOIDExt]"/>
	  	...
	  	<xsl:element name="{$elementName}">
			...
			<xsl:if test="$locationInformationIDs">
				<OtherIdentifiers>
					<xsl:for-each select="$locationInformationIDs">
			...
			<Organization>
				...
				<xsl:if test="$facilityInformationIDs">
					<OtherIdentifiers>
						<xsl:for-each select="$facilityInformationIDs">
							<xsl:if test="string-length(@root)">
								<Identifier>
									<Number><xsl:value-of select="@root"/></Number>
									<xsl:if test="string-length(@extension)">
				...
				<xsl:choose>	
					<!--HealthCareFacility datatype gets additional ParentOrganization tags-->
					<xsl:when test="$elementName='HealthCareFacility' and string-length(.//hl7:wholeOrganization) > 0"> 
							<xsl:apply-templates select=".//hl7:wholeOrganization" mode="fn-OrganizationDetail">
								<xsl:with-param name="emitElementName" select="'ParentOrganization'"/>
							</xsl:apply-templates>
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="$elementName='HealthCareFacility' and .//hl7:asOrganizationPartOf">
						<!-- If we do not have wholeOrganization, try using asOrganizationPartOf -->
							<ParentOrganization>
								<Code>
									<xsl:value-of select=".//hl7:asOrganizationPartOf/hl7:id/@root"/>
								</Code>
								<Description>
									<xsl:value-of select=".//hl7:asOrganizationPartOf/hl7:id/@extension"/>
								</Description>
							</ParentOrganization>
						</xsl:if>
					</xsl:otherwise>
			...
			<xsl:if test="$elementName='HealthCareFacility'"> <!--extra fields for HealthCareFacility only -->
				<xsl:apply-templates select=".//hl7:serviceProviderOrganization/hl7:addr | .//hl7:representedOrganization/hl7:addr" mode="fn-T-pName-address"/>
				<xsl:apply-templates select=".//hl7:serviceProviderOrganization/hl7:telecom | .//hl7:representedOrganization/hl7:telecom" mode="fn-T-pName-ContactInfo"/>
				<xsl:if test=".//hl7:standardIndustryClassCode">
						<Type>
							<Code>
								<xsl:value-of select=".//hl7:standardIndustryClassCode/@code"/>
							</Code>
							<Description>
								<xsl:value-of select=".//hl7:standardIndustryClassCode/@displayName"/>
							</Description>
							<SDACodingStandard>
								<xsl:value-of select=".//hl7:standardIndustryClassCode/@codeSystem"/>
							</SDACodingStandard>
						</Type>
				</xsl:if>
			</xsl:if>
	...
	<xsl:template match="*" mode="fn-ServiceDeliveryLocation">			
			<Location>
				<LocationDetail>
					<xsl:variable name="playingEntityName">
						<xsl:value-of select="hl7:playingEntity/hl7:name" />
					</xsl:variable>	
					<xsl:choose>
						<xsl:when test="hl7:playingEntity/@classCode='PLC' and string-length($playingEntityName)" >
							<Code><xsl:value-of select="$playingEntityName" /></Code>
							<Description><xsl:value-of select="$playingEntityName" /></Description>
						</xsl:when>
						<xsl:otherwise>
							<Code>unknown</Code>
					...
					<xsl:apply-templates select="hl7:code" mode="fn-CodeTable">
						<xsl:with-param name="hsElementName" select="'LocationTypeCoded'"/>
	...
	<xsl:template match="*" mode="fn-FacilityLocation">			
		<Location>
			<xsl:variable name="playingEntityName">
				<xsl:value-of select="hl7:playingEntity/hl7:name" />
			</xsl:variable>	
			...
			<LocationDetail>
				<Organization>
					<xsl:apply-templates select="hl7:participant[@typeCode = 'LOC']/hl7:participantRole/hl7:addr | hl7:location/hl7:addr" mode="fn-T-pName-address"/>
					<xsl:apply-templates select="hl7:participant[@typeCode = 'LOC']/hl7:participantRole/hl7:telecom | hl7:location/hl7:telecom" mode="fn-T-pName-ContactInfo"/>
					...
					<xsl:choose>
						<xsl:when test="string-length($orgCode) and not($orgCode = hl7:id[1]/@root)">
						<Code><xsl:value-of select="$orgCode"/></Code>
						<Description><xsl:value-of select="$orgCode"/></Description>
						</xsl:when>
						<xsl:otherwise>
						<Code>unknown</Code>
						<Description>unknown</Description>
	...
	<xsl:template match="*" mode="fn-HealthCareFacilityLocation">
		<xsl:if test="hl7:healthCareFacility/hl7:serviceProviderOrganization/hl7:name">			
			<Location>
				<LocationDetail>
					<xsl:variable name="facilityName">
						<xsl:value-of select="hl7:healthCareFacility/hl7:serviceProviderOrganization/hl7:name" />
					</xsl:variable>	
					<xsl:choose>
						<xsl:when test="hl7:healthCareFacility" >
							<Code><xsl:value-of select="$facilityName" /></Code>
							<Description><xsl:value-of select="$facilityName" /></Description>
						</xsl:when>
						<xsl:otherwise>
							<Code>unknown</Code>
							<Description>unknown</Description>
					...
					<xsl:apply-templates select="hl7:healthCareFacility/hl7:code" mode="fn-CodeTable">
						<xsl:with-param name="hsElementName" select="'LocationTypeCoded'"/>
						<xsl:with-param name="importOriginalText" select="'1'"/>
					</xsl:apply-templates>
					<xsl:apply-templates select="hl7:healthCareFacility/hl7:location/hl7:addr" mode="fn-T-pName-address"/>
					<xsl:apply-templates select="." mode="fn-T-pName-ContactInfo"/>								
				</LocationDetail>
			</Location>
		</xsl:if>
	...
	<xsl:template match="*" mode="fn-OrganizationDetail">
	  <!-- Match could be hl7:recordTarget, hl7:assignedEntity, hl7:author, hl7:informant -->
	  <xsl:param name="emitElementName" select="'EnteredAt'"/>
		...
		-->
		<xsl:variable name="facilityInformation"><xsl:apply-templates select=".//hl7:representedOrganization | .//hl7:providerOrganization | .//hl7:wholeOrganization" mode="fn-P-FacilityInformation"/></xsl:variable>						
		...
		<xsl:if test="string-length($facilityInformation)">
			<xsl:element name="{$emitElementName}">
				...
				<xsl:choose>
					<xsl:when test=".//hl7:representedOrganization/hl7:addr">
						<xsl:apply-templates 
						select=".//hl7:representedOrganization/hl7:addr" 
						mode="fn-T-pName-address"/>
					</xsl:when>
					<xsl:when test=".//hl7:providerOrganization/hl7:addr">
						<xsl:apply-templates 
						select=".//hl7:providerOrganization/hl7:addr" 
						mode="fn-T-pName-address"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates 
						select=".//hl7:wholeOrganization/hl7:addr" 
						mode="fn-T-pName-address"/>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:choose>
					<xsl:when test=".//hl7:representedOrganization">
						<xsl:apply-templates 
						select=".//hl7:representedOrganization" 
						mode="fn-T-pName-ContactInfo"/>
					</xsl:when>
					<xsl:when test=".//hl7:providerOrganization">
						<xsl:apply-templates 
						select=".//hl7:providerOrganization" 
						mode="fn-T-pName-ContactInfo"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates 
						select=".//hl7:wholeOrganization" 
						mode="fn-T-pName-ContactInfo"/>
					</xsl:otherwise>
				</xsl:choose>
				
				<xsl:if test="not(self::hl7:assignedEntity or self::hl7:author or self::hl7:informant)">
				<!-- Only populate Type and ParentOrganization for other elements -->
					<xsl:if test="./hl7:standardIndustryClassCode">
						<Type>
							<Code>
								<xsl:value-of select="./hl7:standardIndustryClassCode/@code"/>
					...
					<xsl:choose>
						<xsl:when test="./hl7:asOrganizationPartOf/hl7:wholeOrganization">
						<!-- If we have wholeOrganization, recursively create a ParentOrganization entry -->
							<xsl:apply-templates select="./hl7:asOrganizationPartOf/hl7:wholeOrganization" mode="fn-OrganizationDetail">
								<xsl:with-param name="emitElementName" select="'ParentOrganization'"/>
							</xsl:apply-templates>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="./hl7:asOrganizationPartOf">
							<!-- If we do not have wholeOrganization, try using asOrganizationPartOf -->
								<ParentOrganization>
									<Code>
										<xsl:value-of select="./hl7:asOrganizationPartOf/hl7:id/@root"/>
	...
	<xsl:template match="*" mode="fn-EnteredByDetail">
	  <!-- Match is same as fn-EnteredBy -->
		<!--
			...
			Path  : Code
			XPath : Code/text()
			Source: assignedAuthor/id/@extension
			Note  : If id/@extension is not available then assignedPerson/name/text() is used.

			Field
			Path  : SDACodingStandard
			XPath : SDACodingStandard/text()
			Source: assignedAuthor/id/@root
			
			Field
			Path  : Description
			XPath : Description/text()
			Source: assignedAuthor/assignedPerson/name/text()
			
			Field
			Path  : Name
			XPath : Name
			Source: assignedAuthor/name
		...
		<xsl:choose>
			<xsl:when test="hl7:assignedAuthor/hl7:assignedPerson and not(hl7:assignedAuthor/hl7:assignedPerson/hl7:name/@nullFlavor)">	
				<EnteredBy>
					<xsl:variable name="authorAssigningAuthority"><xsl:apply-templates select="." mode="fn-code-for-oid"><xsl:with-param name="OID" select="hl7:assignedAuthor/hl7:id/@root"/></xsl:apply-templates></xsl:variable>
					<xsl:variable name="authorID" select="hl7:assignedAuthor/hl7:id/@extension"/>
					<xsl:variable name="authorName"><xsl:apply-templates select="hl7:assignedAuthor/hl7:assignedPerson/hl7:name" mode="fn-ContactNameString"/></xsl:variable>
						
					<xsl:if test="string-length($authorAssigningAuthority) and not($authorAssigningAuthority=$noCodeSystemName) and not($authorAssigningAuthority=$noCodeSystemOID)">
						<SDACodingStandard><xsl:value-of select="$authorAssigningAuthority"/></SDACodingStandard>
					</xsl:if>
					<Code>
						<xsl:choose>
							<xsl:when test="string-length($authorID)"><xsl:value-of select="$authorID"/></xsl:when>
							<xsl:otherwise><xsl:value-of select="$authorName"/></xsl:otherwise>
						</xsl:choose>
					</Code>
					<Description><xsl:value-of select="$authorName"/></Description>
			...
			<xsl:when test="./hl7:name">
				<EnteredBy>					
					...
					<!-- Contact Name and Contact Information -->
					<xsl:apply-templates select="./hl7:name" mode="fn-T-pName-ContactName"/>
					<xsl:apply-templates select="./hl7:addr" mode="fn-T-pName-address"/>
					<xsl:apply-templates select="." mode="fn-T-pName-ContactInfo"/>
				</EnteredBy>					
			</xsl:when>
	...
	<xsl:template match="hl7:author" mode="fn-Performer">
		<xsl:choose>
			<xsl:when test="hl7:assignedAuthor/hl7:assignedPerson and not(hl7:assignedAuthor/hl7:assignedPerson/hl7:name/@nullFlavor)">	
				<Performer>
					<xsl:choose>
	...
	<xsl:template match="*" mode="fn-EncounterParticipants">
		<!--
			StructuredMapping: EncounterParticipants
			
			Field
			Path  : Participants
			XPath : ./Participants
			Source: ./encounterParticipant
		-->
		<xsl:apply-templates select="hl7:encounterParticipant[@typeCode] | hl7:performer[@typeCode]" mode="fn-EncounterParticipant"/>
	</xsl:template>
	
	<xsl:template match="*" mode="fn-EncounterParticipant">
		<!--
			StructuredMapping: EncounterParticipant
			
			Field
			Path  : Type
			XPath : ./Type
			Source: ./encounterParticipant
			StrucuredMappingRef: ParticipantType
		...
		<EncounterParticipant>
			<xsl:apply-templates select="hl7:time/hl7:low" mode="fn-FromTime"/>
			<xsl:apply-templates select="hl7:time/hl7:high" mode="fn-ToTime"/>
			<xsl:choose>
				<xsl:when test="sdtc:functionCode">
					<xsl:apply-templates select="sdtc:functionCode" mode="fn-CodeTable">
						<xsl:with-param name="hsElementName" select="'Type'"/>
						<xsl:with-param name="importOriginalText" select="'1'"/>
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>
					<xsl:apply-templates select="." mode="fn-ParticipantType"/>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:apply-templates select="." mode="fn-W-CareProvider"/>
		</EncounterParticipant>
	</xsl:template>
	
	<xsl:template match="*" mode="fn-ParticipantType">
		<!--
			StructuredMapping: ParticipantType
			...
			XPath : Description
			Source: @typeCode
			Note  : @typeCode is mapped to a text Description value if @typeCode is
					recognized: Attending Clinician (ATND, PRF), Referring Clinician,
					Consulting Clinician (CON), or Admitting Clinician (ADM).
					Otherwise the Description is the same as the Code.
		-->
		<Type>
			<xsl:choose>
				<xsl:when test="name(self::node())='encounterParticipant' and @typeCode = 'ATND'">	
					<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
					<Code>ATND</Code>
					<Description>attender</Description>
				</xsl:when>			
				<xsl:when test="name(self::node())='performer' and @typeCode = 'PRF'">	
					<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
					<Code>PPRF</Code>
					<Description>primary performer</Description>
				</xsl:when>			
				<xsl:when test="(name(self::node())='encounterParticipant' and @typeCode = 'ADM')">	
					<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
					<Code>ADM</Code>
					<Description>admitter</Description>
				</xsl:when>			
				<xsl:when test="(name(self::node())='encounterParticipant' and @typeCode = 'CON')">	
					<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
					<Code>CON</Code>
					<Description>consultant</Description>
				</xsl:when>			
				<xsl:when test="(name(self::node())='encounterParticipant' and @typeCode = 'REF')">	
					<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
					<Code>REF</Code>
					<Description>referrer</Description>
				</xsl:when>			
				<xsl:otherwise>
					<Code><xsl:value-of select="@typeCode" /></Code>
					<Description><xsl:value-of select="@typeCode" /></Description>
				</xsl:otherwise>
			</xsl:choose>
		</Type>
	</xsl:template>

    <xsl:template match="hl7:encounter" mode="fn-EncounterParticipants-informants">
    	<xsl:apply-templates 
    	    select="hl7:informant[not(hl7:assignedEntity/hl7:id/@nullFlavor) and hl7:assignedEntity/hl7:id/@root and hl7:assignedEntity/hl7:id/@extension and string-length(hl7:assignedEntity/hl7:assignedPerson/hl7:name)]"
	  		 mode="fn-EncounterParticipant-informant" />
    </xsl:template>

    <xsl:template match="hl7:informant" mode="fn-EncounterParticipant-informant">
      	<EncounterParticipant>
      		<Type>
				<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
				<Code>CON</Code>
	...
	<xsl:template match="hl7:performer | hl7:author" mode="fn-Clinician">
		<!--
	...
	<xsl:template match="hl7:author" mode="fn-OrderedBy-Author">
		<xsl:if test="string-length(hl7:assignedAuthor/hl7:id/@extension) or hl7:assignedAuthor/hl7:code or hl7:assignedAuthor/hl7:assignedPerson/hl7:name">
			<OrderedBy>
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:if test="string-length($contactNameFull)">
							<Description><xsl:value-of select="$contactNameFull"/></Description>
						</xsl:if>
	...
	<xsl:template match="hl7:encounterParticipant | hl7:informant | hl7:performer | hl7:author" mode="fn-CareProviderDetail">
		...
		-->
		<xsl:variable name="entityPath" select="hl7:assignedEntity | hl7:associatedEntity | hl7:assignedAuthor"/>
		<xsl:variable name="validQualifiers" select="'|AA|AAS|ABA|AE|AS|BA|BBA|BE|BFA|BN|BS|BSL|BSN|BT|CANP|CER|CMA|CNM
			|CNP|CNS|CPNP|CRN|CTR|DBA|DED|DIP|DO|EMT|EMTP|FPNP|HS|JD|MA|MBA|MCE|MD|MDA|MDI|ME|MED|MEE|MFA|MME|MS|MSL|MSN
			|MT|MTH|NG|NP|PA|PHD|PHE|PHS|PN|PharmD|RMA|RN|RPH|SEC|TS|'" />
		...
		<xsl:if test="$entityPath = true()">
			...
			<xsl:if test="$personPath = true() and not($personPath/hl7:name/@nullFlavor)">
				...
				<xsl:variable name="qualifier" select="$personPath/hl7:name/hl7:suffix/@qualifier" />
				<xsl:variable name="desc" select="$personPath/sdtc:desc" />
				<xsl:choose>
					<xsl:when test="string-length($qualifier) > 0 and contains($validQualifiers, $qualifier)">
						<Qualification>
							<xsl:value-of select="$personPath/hl7:name/hl7:suffix"/>
						</Qualification>
					...
					<xsl:otherwise>
						<xsl:if test="string-length($desc) > 0 and contains($validQualifiers, $desc)">
							<Qualification>
								<xsl:value-of select="$desc"/>
							</Qualification>                        
						</xsl:if>
				...
				<xsl:if test="$entityPath/hl7:id[(position() != 1)]">
					<OtherIdentifiers>
						<xsl:for-each select="$entityPath/hl7:id[(position() != 1)]">
							<xsl:if test="string-length(@root)">
								<Identifier>
									<Number><xsl:value-of select="@root"/></Number>
									<xsl:if test="string-length(@extension)">
	...
	<xsl:template match="hl7:name" mode="fn-T-pName-ContactName">
		...
		<!--
			...
			Path  : FromTime
			XPath : FromTime/text()
			Source: validTime/low/text()
			
			Field
			Path  : ToTime
			XPath : ToTime/text()
			Source: validTime/high/text()
		-->
		<xsl:if test="not(@nullFlavor)">
		  ...
		  <xsl:element name="{$emitElementName}">
				...
				<xsl:choose>
				<xsl:when test="string-length($nameQualifier) and (translate($nameQualifier,$lowerCase,$upperCase) = 'BR')"><Type>Birth</Type></xsl:when>
				<xsl:when test="@use = 'L' or not(@use)"><Type>Legal</Type></xsl:when>
				<xsl:when test="@use = 'P'"><Type>Alias</Type></xsl:when>
				</xsl:choose>
				<xsl:apply-templates select="hl7:validTime/hl7:low" mode="fn-FromTime"/>
				<xsl:apply-templates select="hl7:validTime/hl7:high" mode="fn-ToTime"/>				
			</xsl:element>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="hl7:representedOrganization | hl7:serviceProviderOrganization | hl7:providerOrganization" mode="fn-P-FacilityInformation">
	...
	<!--
		...
		<!-- Response format:  |F0:result flags|F1:Facility OID|F2:Facility Code|F3:Facility Description|F4:Facility Code From ID -->
		<xsl:value-of select="concat('|F0:', $availabilityCode, '|F1:', $facilityOID, '|F2:', $facilityCode, '|F3:', $facilityDescription, '|F4:', $facilityCodeFromIDs, '|')"/>
	</xsl:template>
	...
	<xsl:template match="hl7:healthCareFacility" mode="fn-LocationInformation">
		...
		<xsl:variable name = "idRoot">
			<xsl:value-of select="hl7:id[not(@displayable='true')][1]/@root"/>
		</xsl:variable>
		<xsl:variable name = "idExt">
			<xsl:value-of select="hl7:id[not(@displayable='true')][1]/@extension"/>
		</xsl:variable>
		...
		<!-- Response format:  |L0:result flags|L1:Location OID|L2:Location Code|L3:Location Description|L4:ID Root|L5:ID Extension| element local name -->
		<xsl:value-of select="concat('|L0:', $availabilityCode, '|L1:', $locationOID, '|L2:', $locationCode, '|L3:', $locationDescription, '|L4:', $idRoot, '|L5:', $idExt, '|',local-name())"/>
	</xsl:template>
	...
	<xsl:template match="hl7:id" mode="fn-SdaId">
		<SdaID>
			<xsl:choose>
	...
	<xsl:template name="fn-S-timestamp">
		...
		<!-- Call isc:evaluate('xmltimestamp',$inputString) 
		     Input has the format YYYYMMDDHHNNSSpZZZZ, where all uppercase letters stand for digits and 
		     p stands for either a plus sign or minus sign. Valid input could be as short as YYYYMM.
		     Zeroes are supplied for time-of-day if some or all of the time fields are missing. -->
		<xsl:value-of select="isc:evaluate('xmltimestamp',$inputString)"/>
	</xsl:template>
  ...
  <xsl:template name="fn-ValueRange">
	<!--
		Field: Observed Value Range
		Target: /Container/SocialHistories/SocialHistory/SocialHabitValueRange
		Target: /Container/VitalSigns/VitalSign/ReferenceRange
		Source: /ClinicalDocument/component/structuredBody/component/section[templateId/@root='2.16.840.1.113883.10.20.22.2.17']/entry/observation/value@xsi:type
				/ClinicalDocument/component/structuredBody/component/section[templateId/@root='2.16.840.1.113883.10.20.22.2.4']/entry/observation/value@xsi:type
		Note: 	If value/@xsi:type equals 'IVL_PQ', extract low and high values and map them to the specified target element.
				Comparator for high is set to <= by default; if @inclusive='false', set comparator to <.
				Comparator for low is set to >= by default; if @inclusive='false', set comparator to >.
	-->
		<xsl:param name="elementName" />
		...
		<xsl:element name="{$elementName}">
			<xsl:if test="$highValue">
				<High>
					...
					<Comparator>
						<xsl:choose>
							<xsl:when test="hl7:value/hl7:high/@inclusive = 'false'">
								<xsl:text>&lt;</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:text>&lt;=</xsl:text>
					...
					<xsl:if test="hl7:value/hl7:high/@unit">
						<UnitOfMeasure>
							<Code>
			...
			<xsl:if test="$lowValue">
				<Low>
					...
					<Comparator>
						<xsl:choose>
							<xsl:when test="hl7:value/hl7:low/@inclusive = 'false'">
								<xsl:text>&gt;</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:text>&gt;=</xsl:text>
					...
					<xsl:if test="hl7:value/hl7:low/@unit">
						<UnitOfMeasure>
							<Code>
	...
	<xsl:template match="*" mode="fn-Identifiers">
		<xsl:if test="hl7:id[not(@nullFlavor)]">
			<Identifiers>
						...
						<xsl:choose>
							<xsl:when test="string-length(@extension) > 0">
								...
								<xsl:if test="string-length(@root) > 0">	
									<ISOAssigningAuthority><xsl:value-of select="@root" /></ISOAssigningAuthority>	
								</xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<Number><xsl:value-of select="@root" /></Number>
