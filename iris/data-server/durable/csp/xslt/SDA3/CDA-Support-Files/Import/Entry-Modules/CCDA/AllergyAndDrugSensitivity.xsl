...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com" xmlns:hl7="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:exsl="http://exslt.org/common" extension-element-prefixes="exsl" exclude-result-prefixes="isc hl7 xsi exsl">
	...
	<xsl:template match="*" mode="Allergy">
		<Allergy>
			...
			<!-- Allergy Reaction -->
			<xsl:apply-templates select="hl7:entryRelationship[@typeCode='SUBJ']/hl7:observation/hl7:entryRelationship[@typeCode='MFST']" mode="AllergyReaction"/>

			<!-- Allergy Severity -->
			<xsl:apply-templates select="hl7:entryRelationship[@typeCode='SUBJ']/hl7:observation/hl7:entryRelationship/hl7:observation[hl7:templateId/@root=$ccda-SeverityObservation]" mode="AllergySeverity"/>

			<!-- Allergy Status -->
			<xsl:apply-templates select="hl7:entryRelationship[@typeCode='SUBJ']/hl7:observation/hl7:entryRelationship/hl7:observation[hl7:templateId/@root=$ccda-AllergyStatusObservation]" mode="AllergyStatus"/>
	...
	<xsl:template match="*" mode="AllergyStatus">
		...
		<Status>
			<xsl:choose>
				<xsl:when test="hl7:value/@code = '55561003'">A</xsl:when>
	...
	<xsl:template match="*" mode="AllergyReaction">
		...
		-->
  		<xsl:choose>
    		...
    		<xsl:otherwise>
      			<xsl:apply-templates select="hl7:observation/hl7:text" mode="CodeTable">
        			<xsl:with-param name="hsElementName" select="'Reaction'"/>
      			</xsl:apply-templates>
    		</xsl:otherwise>
  		</xsl:choose>
