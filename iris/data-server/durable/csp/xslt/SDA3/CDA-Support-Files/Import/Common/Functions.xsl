...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:isc="http://extension-functions.intersystems.com" xmlns:hl7="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="isc hl7 sdtc xsi exsl">
	...
	<xsl:template match="*" mode="SendingFacilityOID">
		<xsl:choose>
			<xsl:when test="$defaultInformantRootPath/hl7:assignedEntity/hl7:representedOrganization"><xsl:apply-templates select="$defaultInformantRootPath" mode="SendingFacilityOIDFromOrgInfo"/></xsl:when>
			<xsl:when test="$defaultAuthorRootPath/hl7:assignedAuthor/hl7:representedOrganization"><xsl:apply-templates select="$defaultAuthorRootPath" mode="SendingFacilityOIDFromOrgInfo"/></xsl:when>
			<xsl:otherwise><xsl:apply-templates select="$defaultAuthoringDeviceRootPath" mode="SendingFacilityOIDFromOrgInfo"/></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="*" mode="SendingFacilityOIDFromOrgInfo">
		<xsl:variable name="organizationInformation"><xsl:apply-templates select=".//hl7:representedOrganization" mode="OrganizationInformation"/></xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="string-length($facilityOID)"><xsl:value-of select="$facilityOID"/></xsl:when>
			<xsl:otherwise>Unknown</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="*" mode="ServiceDeliveryLocation">			
			<Location>
				<LocationDetail>
	...
	<xsl:template match="*" mode="HealthCareFacilityLocation">			
			<Location>
				<LocationDetail>
					...
					<xsl:apply-templates select="hl7:healthCareFacility/hl7:code" mode="CodeTable">
						<xsl:with-param name="hsElementName" select="'LocationTypeCoded'"/>
						<xsl:with-param name="importOriginalText" select="'1'"/>
					</xsl:apply-templates>
					<xsl:apply-templates select="hl7:healthCareFacility/hl7:location/hl7:addr" mode="Address"/>
					<xsl:apply-templates select="." mode="ContactInfo"/>								
				</LocationDetail>
			</Location>
	</xsl:template>
	...
	<xsl:template match="*" mode="EncounterParticipants">
		<!--
		...
		-->
		<xsl:apply-templates select="hl7:encounterParticipant[@typeCode] | hl7:performer[@typeCode]" mode="EncounterParticipant"/>
	</xsl:template>
	
	<xsl:template match="*" mode="EncounterParticipant">
		<!--
		...
		<EncounterParticipant>
			<xsl:apply-templates select="hl7:time/hl7:low" mode="FromTime"/>
			<xsl:apply-templates select="hl7:time/hl7:high" mode="ToTime"/>
			<xsl:apply-templates select="." mode="ParticipantType"/>
			<xsl:apply-templates select="." mode="CareProvider"/>
		</EncounterParticipant>
	</xsl:template>
	
	<xsl:template match="*" mode="ParticipantType">
		<!--
    ...
    <xsl:template match="hl7:encounter" mode="EncounterParticipants-informants">
    	<xsl:apply-templates 
    	    select="hl7:informant[not(hl7:assignedEntity/hl7:id/@nullFlavor) and hl7:assignedEntity/hl7:id/@root and hl7:assignedEntity/hl7:id/@extension and string-length(hl7:assignedEntity/hl7:assignedPerson/hl7:name)]"
	  		 mode="EncounterParticipant-informant" />
    </xsl:template>

    <xsl:template match="hl7:informant" mode="EncounterParticipant-informant">
      	<EncounterParticipant>
      		<Type>
				<SDACodingStandard>http://terminology.hl7.org/CodeSystem/v3-ParticipationType</SDACodingStandard>
				<Code>CON</Code>
	...
	<xsl:template match="*" mode="DoctorDetail">
		...
		<xsl:if test="$entityPath = true()">
			...
			<xsl:if test="$personPath = true() and not($personPath/hl7:name/@nullFlavor)">
				...
				<xsl:if test="string-length($clinicianAssigningAuthority) and not($clinicianAssigningAuthority=$noCodeSystemName) and not($clinicianAssigningAuthority=$noCodeSystemOID)"><SDACodingStandard><xsl:value-of select="$clinicianAssigningAuthority"/></SDACodingStandard></xsl:if>
				<Code>
					<xsl:choose>
						<xsl:when test="string-length($clinicianID)"><xsl:value-of select="$clinicianID"/></xsl:when>
						<xsl:otherwise><xsl:value-of select="$clinicianName"/></xsl:otherwise>
	...
	<!-- AddressType imports to HS.Types.Address instead of to HS.SDA3.Address. -->
			...
			<xsl:otherwise>
				<xsl:element name="{$elementName}">
					...
					<xsl:if test="hl7:city">
						<City>
							<xsl:value-of select="normalize-space(hl7:city/text())"/>
						</City>
					</xsl:if>
					<xsl:if test="hl7:state">
						<State>
							<xsl:value-of select="normalize-space(hl7:state/text())"/>
						</State>
					</xsl:if>
					<xsl:if test="hl7:postalCode">
						<PostalCode>
							<xsl:value-of select="normalize-space(hl7:postalCode/text())"/>
						</PostalCode>
					</xsl:if>
					<xsl:if test="hl7:country">
						<Country>
							<xsl:value-of select="normalize-space(hl7:country/text())"/>
						</Country>
					</xsl:if>
					<xsl:if test="hl7:county">
						<County>
							<xsl:value-of select="normalize-space(hl7:county/text())"/>
						</County>
					</xsl:if>
				</xsl:element>
			</xsl:otherwise>
	...
	<xsl:template match="*" mode="XFRMAllEncounters">
		...
		<xsl:apply-templates select="$sectionRootPath[hl7:templateId/@root=$encountersSectionTemplateId]/hl7:entry" mode="XFRMOverriddenEncounter">
			<xsl:with-param name="informationType" select="$informationType"/>
			<xsl:with-param name="actionScope" select="$actionScope"/>
		</xsl:apply-templates>
	</xsl:template>
