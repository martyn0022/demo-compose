...
<!--
  ...
  Revision History:  08/21/13 Paul Lomayesva (InterSystems Corporation) updated
      - Added display of Contact Info for serviceEvent Performers (Care Team)
      - Modified display of Patient IDs to show assigningAuthorityName instead of root when assigningAuthorityName is available
      - Modified template show-contactinfo to display a one-line message "Address and Telecom information not available" when both hl7:addr and hl7:telecom are either absent or nullFlavor
      - Corrected the spelling "Pirmary Home" to "Primary Home"
      - Changed the display of telecom to display "Unknown type" when the telecom code is not in the list of known valid codes
      - Changed the display of performer to display "Unknown role" when the performer classCode is not in the list of known valid performer classCodes
      - Changed the display of support contact to display "(Unknown role)" when the support contact code is not in the list of known valid codes
  Revision History:  08/23/13 Paul Lomayesva (InterSystems Corporation) updated
      - Added Preferred Language to demographic information section
  Revision History:  09/15/13 Paul Lomayesva (InterSystems Corporation) Added template lineFeedToBR, and added calls to it in n1:td, n1:list, and n1:content.
  Revision History:  12/30/13 Paul Lomayesva (InterSystems Corporation) Fixed the handling of styleCode, which never worked.
  Revision History:  01/23/14 Paul Lomayesva (InterSystems Corporation) Incorporated Rick Geimer's (Lantana Group) suggested code for rending embedded multimedia from nonXMLBody.
  Revision History:  04/10/14 Paul Lomayesva (InterSystems Corporation) Incorporated Rick Geimer's (Lantana Group) fixes for security vunerabilities and other misc minor changes from Rick.
  Revision History:  09/01/14 Paul Lomayesva (InterSystems Corporation) Corrected checks for line feeds to add a check for text under children (text()=descendant-or-self::node()).
...
<xsl:stylesheet version="1.0"
      ...
      <html>
         ...
         <body>
          ...
          ` <xsl:call-template name="documentGeneral"/>
            ...
            <xsl:call-template name="encounterAuthor"/>
   ...
   <!-- participantENT -->
   <xsl:template name="encounterAuthor">
      <xsl:variable name="author" select="/n1:ClinicalDocument/n1:component/n1:structuredBody/n1:component/n1:section/n1:entry/n1:encounter/n1:author[@typeCode='AUT']"/>
      <xsl:if test="$author">
         <table class="header_table">
            <tbody>
               <tr>
                  ...
                  <td width="80%">
                     <xsl:call-template name="show-name">
                        <xsl:with-param name="name" select="$author/n1:assignedAuthor/n1:assignedPerson/n1:name"/>
                     </xsl:call-template>                     
                  </td>
               </tr>
               <xsl:if test="$author/n1:assignedAuthor/n1:addr | $author/n1:assignedAuthor/n1:telecom">
                  <tr>
                     <td bgcolor="#3399ff">
                        <span class="td_label">
                           <xsl:text>Contact info</xsl:text>
                     ...
                     <td>
                        <xsl:call-template name="show-contactInfo">
                           <xsl:with-param name="contact" select="$author/n1:assignedAuthor/n1:representedOrganization"/>
                        </xsl:call-template>
                     </td>
                  </tr>
               </xsl:if>               
            ...
            <xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole">
               <xsl:if test="not(n1:id/@nullFlavor)">
                  <tr>
                     ...
                     <td>
                        <xsl:for-each select="n1:patient/n1:name[@use='P']">
                           <xsl:if test="position() &gt; 1">
                              <br/>
                           </xsl:if>
                           <xsl:call-template name="show-alias">
                  ...
                  <xsl:if test="n1:patient/n1:languageCommunication[n1:preferenceInd/@value='true']">
                       <tr>
                         ...
                         <td>
                            <!-- Translation is based on ISO 639-2.  -->
                            <xsl:variable name="langCode" select="n1:patient/n1:languageCommunication[n1:preferenceInd/@value='true']/n1:languageCode/@code"/>
                            <xsl:choose>
                               <xsl:when test="$langCode='eng' or $langCode='en'">English</xsl:when>
                               <xsl:when test="$langCode='spa'">Spanish</xsl:when>
                               <xsl:when test="$langCode='fra' or $langCode='fre'">French</xsl:when>
                               <xsl:when test="$langCode='jpn'">Japanese</xsl:when>
                               <xsl:when test="$langCode='chi' or $langCode='zho'">Chinese</xsl:when>
                               ...
                               <xsl:when test="$langCode='bur' or $langCode='mya'">Burmese</xsl:when>
                               <xsl:when test="$langCode='haw'">Hawaiian</xsl:when>
                               <xsl:when test="$langCode='gla'">Gaelic</xsl:when>
                               <xsl:when test="$langCode='ger' or $langCode='due'">German</xsl:when>
                               <xsl:when test="$langCode='ita'">Italian</xsl:when>
                               <xsl:when test="$langCode='dut' or $langCode='nld'">Dutch</xsl:when>
                               <xsl:when test="$langCode='gsw'">Swiss German</xsl:when>
                               <xsl:when test="$langCode='por'">Portuguese</xsl:when>
                               <xsl:when test="$langCode='tur'">Turkish</xsl:when>
                               <xsl:when test="$langCode='aus'">Australian</xsl:when>
                               <xsl:when test="$langCode='sco'">Scots</xsl:when>
                               <xsl:when test="$langCode='wln'">Walloon</xsl:when>
                               <xsl:when test="$langCode='wel' or $langCode='cym'">Welsh</xsl:when>
                               ...
                               <xsl:when test="$langCode='gre' or $langCode='ell'">Greek</xsl:when>
                               <xsl:when test="$langCode='cze' or $langCode='ces'">Czech</xsl:when>
                               <xsl:when test="$langCode='rum' or $langCode='ron'">Romanian</xsl:when>
   ...
   <xsl:template match='n1:component/n1:nonXMLBody'>
      <xsl:choose>
         ...
         <xsl:when test="n1:text/@representation='B64'">
            <object name='nonXMLBody' id='nonXMLBody' width='80%' height='600' >
               <xsl:attribute name="type"><xsl:value-of select="n1:text/@mediaType"/></xsl:attribute>
               <xsl:attribute name="data"><xsl:text>data:</xsl:text><xsl:value-of select="n1:text/@mediaType"/><xsl:text>;base64,</xsl:text><xsl:value-of select="n1:text"/></xsl:attribute>
            </object>
        </xsl:when>
        <xsl:otherwise>
   ...
   <xsl:template match="n1:content">
      <xsl:choose>
         <xsl:when test="contains(text(),'&#10;') and string-length(translate(text(),'&#10;&#9; ','')) and text()=descendant-or-self::node()">
   ...
   <!--
   		Create a list of valid CDA R2 HTML attributes
   		and use the list plus the output-attrs template
   		to copy over only the valid attributes.
   -->
   ...
   <xsl:template name="show-alias">
      ...
      <xsl:choose>
         <xsl:when test="$name/n1:family">
            ...
            <xsl:text> </xsl:text>
            <xsl:value-of select="$name/n1:family"/>
            <xsl:if test="$name/n1:family/@qualifier='BR' and $name/n1:given/@qualifier='BR'">
               <xsl:text> (Birth Name)</xsl:text>
            </xsl:if>            
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="$name"/>
   ...
   <xsl:template name="show-contactInfo">
      ...
      <xsl:choose>
         <xsl:when test="(not($contact/n1:addr) or $contact/n1:addr/@nullFlavor) and (not($contact/n1:telecom) or $contact/n1:telecom/@nullFlavor)">Address and Telecom information not available</xsl:when>
         <xsl:otherwise>
   ...
   <xsl:template name="show-address">
      ...
      <xsl:choose>
         <xsl:when test="$address and not($address/@nullFlavor)">
