...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:ext="http://ns.electronichealth.net.au/Ci/Cda/Extensions/3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<xsl:template match="*" mode="referrals">
		...
		<xsl:variable name="hasData" select="Referrals/Referral"/>
		
		<xsl:if test="($hasData) or ($sectionRequired='1')">
			<component>
				<section>					
					<!-- IHE needs unique id for each and every section -->
					<id root="{$homeCommunityOID}" extension="{isc:evaluate('createUUID')}"/>
					
					<code code="102.16347" codeSystem="{$nctisOID}" codeSystemName="{$nctisName}" displayName="Referral Detail"/>
					<title>Referral Detail</title>
					
					<!-- Referrals - requires one and only one -->
					<xsl:choose>
						<xsl:when test="$hasData">
							<xsl:apply-templates select="." mode="referral-Narrative"/>
							<xsl:apply-templates select="." mode="referral-Entries"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:apply-templates select="." mode="referral-NoData"/>
						</xsl:otherwise>
					</xsl:choose>
				</section>
			</component>
