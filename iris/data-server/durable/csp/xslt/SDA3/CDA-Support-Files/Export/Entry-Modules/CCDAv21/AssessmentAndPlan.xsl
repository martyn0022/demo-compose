...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
  ...
  <xsl:template match="Container" mode="eANP-assessmentAndPlan-Narrative">
    <xsl:param name="docs"/>
    ...
    <text>
      <table border="1" width="100%">
        ...
        <tbody>
          <xsl:apply-templates select="Documents/Document[DocumentType/Description/text()='AssessmentAndPlan' or DocumentType/Code/text()='AssessmentAndPlan']" mode="eANP-assessmentAndPlan-NarrativeDetail"/>
        </tbody>
      </table>
      ...
      <xsl:apply-templates mode="narrative-export-documents" select=".">
        ...
        <xsl:with-param name="header" select="$exportConfiguration/narrative/section[templateId/@root=$ccda-AssessmentAndPlanSection]/title/text()"/>
      </xsl:apply-templates>

    </text>
  </xsl:template>
  
  <xsl:template match="Document" mode="eANP-assessmentAndPlan-NarrativeDetail">
    <xsl:variable name="narrativeLinkSuffix" select="position()"/>		
    
    <xsl:variable name="narrativeDate">
      <xsl:if test="string-length(FromTime/text())">
    ...
    <tr ID="{concat($exportConfiguration/assessmentAndPlan/narrativeLinkPrefixes/assessmentAndPlanNarrative/text(), $narrativeLinkSuffix)}">
      <td ID="{concat($exportConfiguration/assessmentAndPlan/narrativeLinkPrefixes/assessmentAndPlanDescription/text(), $narrativeLinkSuffix)}">
        <xsl:value-of select="DocumentName"/>
      </td>
      <td>
        <xsl:if test="$narrativeDate">
          <xsl:value-of select="substring-before($narrativeDate,' ')"/>
        </xsl:if>
      </td>
      <td ID="{concat($exportConfiguration/assessmentAndPlan/narrativeLinkPrefixes/assessmentAndPlanComments/text(), $narrativeLinkSuffix)}">
        <xsl:value-of select="NoteText/text()"/>
      </td>
    </tr>
  </xsl:template>
  
  <xsl:template match="Container" mode="eANP-assessmentAndPlan-NoData">
    <text><xsl:value-of select="$exportConfiguration/assessmentAndPlan/emptySection/narrativeText/text()"/></text>
  </xsl:template>
  
  <xsl:template match="Container" mode="eANP-assessmentAndPlan-Entries">
    <xsl:apply-templates select="Documents/Document[DocumentType/Code/text()='AssessmentAndPlan' or DocumentType/Description/text()='AssessmentAndPlan']" mode="eANP-assessmentAndPlan-Entry"/>
  </xsl:template>
  
  <xsl:template match="Document" mode="eANP-assessmentAndPlan-Entry">
    <xsl:variable name="narrativeLinkSuffix" select="position()"/>
    <xsl:apply-templates select="." mode="eANP-assessmentAndPlan-EntryDetail">
      <xsl:with-param name="narrativeLinkSuffix" select="$narrativeLinkSuffix"/>
    </xsl:apply-templates>
  </xsl:template>
  
  <xsl:template match="Document" mode="eANP-assessmentAndPlan-EntryDetail">
    ...
    <entry typeCode="DRIV">
      <act classCode="ACT">
        ...
        <xsl:call-template name="eANP-templateIds-assessmentAndPlanEntry"/>
        
        <!--
          Field : Assessment and Plan Entry id
          Target: /ClinicalDocument/component/structuredBody/component/section[templateId/@root='2.16.840.1.113883.10.20.2.7']/entry/act/id
        ...
        <!--
          ...
          Target: /ClinicalDocument/component/structuredBody/component/section[templateId/@root='2.16.840.1.113883.10.20.2.7']/entry/act/text/reference/@value
        ...
        -->
        <xsl:apply-templates select="CustomPairs" mode="fn-generic-Coded">
  ...
  <xsl:template name="eANP-templateIds-assessmentAndPlanEntry">
