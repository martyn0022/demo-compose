...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<xsl:template match="*" mode="effectiveTime-healthPlan">
		<!--
			HS.SDA3.HealthFund FromTime
			CDA Section: Payers
			CDA Field: Health Plan Effective Date
			CDA XPath: entry/act/effectiveTime
		-->
		<!--
			HS.SDA3.HealthFund ToTime
			CDA Section: Payers
			CDA Field: Health Plan Expiration Date
			CDA XPath: entry/act/effectiveTime
		-->		
		<effectiveTime>			
			<xsl:choose>
	...
	<xsl:template match="*" mode="assignedEntity-healthPlan">
		<assignedEntity>
			...
			<!--
				HS.SDA3.HealthFund HealthFund
				CDA Section: Payers
				CDA Field: Payer
				CDA XPath: entry/act/entryRelationship/act/performer/assignedEntity
			-->			
	...
	<xsl:template match="*" mode="participantRole-healthPlan">
		<participantRole classCode="PAT">
			...
			<!--
				HS.SDA3.HealthFund MembershipNumber
				CDA Section: Payers
				CDA Field: Member ID
				CDA XPath: entry/act/entryRelationship/act/pariticipant/participantRole/id
			-->		
			<xsl:apply-templates select="." mode="id-PayerMember"/>
			
			<!--
				HS.SDA3.HealthFund InsuredRelationship
				CDA Section: Payers
				CDA Field: Patient Relationship to Subscriber
				CDA XPath: entry/act/entryRelationship/act/participant/participantRole
				
				For RoleCode, valid codes are: FAMDEP, HANDIC, INJ, SELF, SPON STUD, FSTUD, PSTUD
			-->			
			<xsl:choose>
	...
	<xsl:template match="*" mode="participantRole-healthPlanSubscriber">
		<participantRole>
			...
			<!--
				HS.SDA3.HealthFund InsuredAddress
				CDA Section: Payers
				CDA Field: Subscriber Address
				CDA XPath: entry/act/entryRelationship/act/participant/participantRole/addr
			-->			
			<xsl:apply-templates select="." mode="address-HomePrimary"/>

			<!--
				HS.SDA3.HealthFund InsuredContact
				CDA Section: Payers
				CDA Field: Subscriber Phone/Email/URL
				CDA XPath: entry/act/entryRelationship/act/participant/participantRole
			-->
			<xsl:apply-templates select="." mode="telecom"/>
	...
	<xsl:template match="*" mode="playingEntity-healthPlan">
		<playingEntity>
			...
			<!--
				HS.SDA3.HealthFund InsuredName
				CDA Section: Payers
				CDA Field: Subscriber Name
				CDA XPath: entry/act/entryRelationship/act/participant/participantRole/playingEntity/name
			-->		
	...
	<xsl:template match="*" mode="code-healthPlanType">
		...
		<!--
			HS.SDA3.HealthFund PlanType
			CDA Section: Payers
			CDA Field: Health Insurance Type
			CDA XPath: entry/act/entryRelationship/act/code
		-->		
		<xsl:apply-templates select="$healthPlanType" mode="generic-Coded">
			<xsl:with-param name="requiredCodeSystemOID" select="$x12n1336OID"/> <!-- X12N-1336 -->
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="*" mode="entryRelationship-healthPlanHealthPlan">
		...
		<!--
			HS.SDA3.HealthFund HealthFundPlan
			CDA Section: Payers
			CDA Field: Health Plan
			CDA XPath: entry/act/entryRelationship/act/entryRelationship
		-->		
