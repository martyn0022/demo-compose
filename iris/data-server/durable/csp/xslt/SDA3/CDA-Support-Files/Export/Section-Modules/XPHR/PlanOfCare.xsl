...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<!--
		This XPHR and XDS-MS version of the Plan of Care section
		header uses LOINC code 61145-9 instead of 18776-5, and
		also has a different displayName for the code element.
	-->
	<xsl:template match="*" mode="planOfCare">
		<xsl:param name="sectionRequired" select="'0'"/>
		
		<xsl:variable name="hasData" select="RadOrders/RadOrder[not(Result)] | LabOrders/LabOrder[not(Result)] | OtherOrders/OtherOrder[not(Result)]"/>
		...
		<xsl:if test="($hasData) or ($exportSectionWhenNoData='1') or ($sectionRequired='1')">
			<component>
				<section>
					...
					<code code="61145-9" displayName="PATIENT PLAN OF CARE" codeSystem="{$loincOID}" codeSystemName="{$loincName}"/>
