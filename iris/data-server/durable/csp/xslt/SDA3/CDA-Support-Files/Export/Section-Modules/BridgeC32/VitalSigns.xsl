...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<xsl:template match="*" mode="vitalSigns-validVitalSign">
		...
		<!-- Use the following line instead, when we decide to support BMI and BSA. -->
	...
	<xsl:template match="*" mode="templateIds-vitalSignsSection">
		<xsl:if test="$hitsp-CDA-VitalSignsSection"><templateId root="{$hitsp-CDA-VitalSignsSection}"/></xsl:if>
		<xsl:if test="$hl7-CCD-VitalSignsSection"><templateId root="{$hl7-CCD-VitalSignsSection}"/></xsl:if>
		<xsl:if test="$ihe-PCC-VitalSigns"><templateId root="{$ihe-PCC-VitalSigns}"/></xsl:if>
		<xsl:if test="$ihe-PCC-CodedVitalSigns"><templateId root="{$ihe-PCC-CodedVitalSigns}"/></xsl:if>		
	</xsl:template>
</xsl:stylesheet>
