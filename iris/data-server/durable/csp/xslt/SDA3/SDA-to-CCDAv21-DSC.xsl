...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<xsl:include href="CDA-Support-Files/Export/Section-Modules/CCDAv21/HospitalCourse.xsl"/>
	<xsl:include href="CDA-Support-Files/Export/Section-Modules/CCDAv21/HospitalDischargeInstructions.xsl"/>
	<xsl:include href="CDA-Support-Files/Export/Section-Modules/CCDAv21/HospitalDischargeMedications.xsl"/>
	...
	<xsl:template match="/Container">
		<ClinicalDocument xmlns="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc">
			...
			<xsl:apply-templates select="." mode="templateId-DSCHeader"/>
			...
			<component>
				<structuredBody>
					...
					<xsl:apply-templates select="." mode="sHC-hospitalCourse">
						<xsl:with-param name="sectionRequired" select="'1'"/>
					</xsl:apply-templates>

					<!-- Discharge Diagnosis module -->
					<xsl:apply-templates select="." mode="sDD-dischargeDiagnoses">
						<xsl:with-param name="sectionRequired" select="'1'"/>
					</xsl:apply-templates>

					<!-- Hospital Discharge Medications module -->
					<xsl:apply-templates select="." mode="sHDM-dischargeMedications">
						<xsl:with-param name="sectionRequired" select="'1'"/>
					</xsl:apply-templates>
					...
					<xsl:apply-templates mode="narrative-export-sections" select=".">
						<xsl:with-param name="templates">
							<item root="{$ccda-HospitalAdmissionMedicationsSectionEntriesOptional}"/>
							<item root="{$ccda-CareTeamsSection}"/>
							<item root="{$ccda-HospitalConsultationsSection}"/>
							<item root="{$ccda-HospitalDischargePhysicalSection}"/>
							<item root="{$ccda-HospitalDischargeStudiesSummarySection}"/>
	...
	<xsl:template match="*" mode="templateId-DSCHeader">
		<templateId root="{$ccda-DischargeSummary}"/>
		<templateId root="{$ccda-DischargeSummary}" extension="2015-08-01"/>		
	</xsl:template>	
