		...
		<xsl:choose>
			<xsl:when test="not(f:kind/@value=&apos;root&apos; and f:uniqueId/f:type/@value=(&apos;uuid&apos;, &apos;ruid&apos;))">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:NamingSystem",</xsl:text>
					<xsl:text>"assert":"not(f:kind/@value=&apos;root&apos; and f:uniqueId/f:type/@value=(&apos;uuid&apos;, &apos;ruid&apos;))",</xsl:text>
					<xsl:text>"text":"nsd-1: Root systems cannot have uuid or sid identifiers",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
