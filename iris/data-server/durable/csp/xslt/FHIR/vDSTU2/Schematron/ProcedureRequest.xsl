...
<!-- *** THIS XSLT STYLESHEET IS DEPRECATED *** -->
	...
	<xsl:template match="/">
		...
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledPeriod"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsQuantity"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange/f:low"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange/f:high"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsPeriod"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:duration"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:period"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:encounter"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:performer"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:notes/f:authorReference"/>
		<xsl:apply-templates select="f:ProcedureRequest/f:orderer"/>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledPeriod">
		<xsl:choose>
			<xsl:when test="not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledPeriod",</xsl:text>
					<xsl:text>"assert":"not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)",</xsl:text>
					<xsl:text>"text":"per-1: If present, start SHALL have a lower value than end",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat">
		<xsl:choose>
			<xsl:when test="not(exists(f:frequency)) or not(exists(f:when))">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat",</xsl:text>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsQuantity">
		<xsl:choose>
			<xsl:when test="not(exists(f:code)) or exists(f:system)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsQuantity",</xsl:text>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange">
		<xsl:choose>
			<xsl:when test="not(exists(f:low/f:value/@value)) or not(exists(f:high/f:value/@value)) or (number(f:low/f:value/@value) &lt;= number(f:high/f:value/@value))">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange",</xsl:text>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange/f:low">
		<xsl:choose>
			<xsl:when test="not(exists(f:code)) or exists(f:system)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange/f:low",</xsl:text>
					<xsl:text>"assert":"not(exists(f:code)) or exists(f:system)",</xsl:text>
					<xsl:text>"text":"qty-3: If a code for the unit is present, the system SHALL also be present",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange/f:high">
		<xsl:choose>
			<xsl:when test="not(exists(f:code)) or exists(f:system)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsRange/f:high",</xsl:text>
					<xsl:text>"assert":"not(exists(f:code)) or exists(f:system)",</xsl:text>
					<xsl:text>"text":"qty-3: If a code for the unit is present, the system SHALL also be present",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsPeriod">
		<xsl:choose>
			<xsl:when test="not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:boundsPeriod",</xsl:text>
					<xsl:text>"assert":"not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)",</xsl:text>
					<xsl:text>"text":"per-1: If present, start SHALL have a lower value than end",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:duration">
		<xsl:choose>
			<xsl:when test="@value &gt;= 0 or not(@value)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:duration",</xsl:text>
	...
	<xsl:template match="f:ProcedureRequest/f:scheduledTiming/f:repeat/f:period">
		<xsl:choose>
			<xsl:when test="@value &gt;= 0 or not(@value)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:scheduledTiming/f:repeat/f:period",</xsl:text>
					<xsl:text>"assert":"@value &gt;= 0 or not(@value)",</xsl:text>
	...
	<xsl:template match="f:ProcedureRequest/f:encounter">
		<xsl:choose>
			<xsl:when test="not(starts-with(f:reference/@value, &apos;#&apos;)) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)])">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:encounter",</xsl:text>
					<xsl:text>"assert":"not(starts-with(f:reference/@value, &apos;#&apos;)) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)])",</xsl:text>
					<xsl:text>"text":"ref-1: SHALL have a local reference if the resource is provided inline",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
	...
	<xsl:template match="f:ProcedureRequest/f:notes/f:authorReference">
		<xsl:choose>
			<xsl:when test="not(starts-with(f:reference/@value, &apos;#&apos;)) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)])">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:notes/f:authorReference",</xsl:text>
					<xsl:text>"assert":"not(starts-with(f:reference/@value, &apos;#&apos;)) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)])",</xsl:text>
					<xsl:text>"text":"ref-1: SHALL have a local reference if the resource is provided inline",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
	...
	<xsl:template match="f:ProcedureRequest/f:orderer">
		<xsl:choose>
			<xsl:when test="not(starts-with(f:reference/@value, &apos;#&apos;)) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)])">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ProcedureRequest/f:orderer",</xsl:text>
					<xsl:text>"assert":"not(starts-with(f:reference/@value, &apos;#&apos;)) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, &apos;#&apos;)])",</xsl:text>
					<xsl:text>"text":"ref-1: SHALL have a local reference if the resource is provided inline",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
