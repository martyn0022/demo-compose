...
<!-- *** THIS XSLT STYLESHEET IS DEPRECATED *** -->
	...
	<xsl:template match="/">
		...
		<xsl:apply-templates select="f:ValueSet/f:codeSystem"/>
		<xsl:apply-templates select="f:ValueSet/f:compose"/>
		...
		<xsl:choose>
			<xsl:when test="not(f:codeSystem/f:system/@value = f:url/@value)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ValueSet",</xsl:text>
					<xsl:text>"assert":"not(f:codeSystem/f:system/@value = f:url/@value)",</xsl:text>
					<xsl:text>"text":"vsd-7: A defined code system (if present) SHALL have a different url than the value set url",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="exists(f:codeSystem) or exists(f:compose) or exists(f:expansion)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ValueSet",</xsl:text>
					<xsl:text>"assert":"exists(f:codeSystem) or exists(f:compose) or exists(f:expansion)",</xsl:text>
					<xsl:text>"text":"vsd-5: Value set SHALL contain at least one of a codeSystem, a compose, or an expansion element",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="not(exists(f:compose)) or (count(f:compose/f:import)!=1 or exists(f:compose/f:include) or exists(f:compose/f:exclude) or exists(f:codeSystem))">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ValueSet",</xsl:text>
					<xsl:text>"assert":"not(exists(f:compose)) or (count(f:compose/f:import)!=1 or exists(f:compose/f:include) or exists(f:compose/f:exclude) or exists(f:codeSystem))",</xsl:text>
					<xsl:text>"text":"vsd-2: A value set with only one import SHALL also have an include and/or an exclude unless the value set includes and inline code system",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="f:ValueSet/f:codeSystem">
		<xsl:choose>
			<xsl:when test="count(descendant::f:concept)=count(distinct-values(descendant::f:concept/f:code/@value))">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ValueSet/f:codeSystem",</xsl:text>
					<xsl:text>"assert":"count(descendant::f:concept)=count(distinct-values(descendant::f:concept/f:code/@value))",</xsl:text>
					<xsl:text>"text":"vsd-8: Codes must be unique",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="count(distinct-values(descendant::f:concept/f:code/@value))=count(descendant::f:concept)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ValueSet/f:codeSystem",</xsl:text>
					<xsl:text>"assert":"count(distinct-values(descendant::f:concept/f:code/@value))=count(descendant::f:concept)",</xsl:text>
					<xsl:text>"text":"vsd-3: Within a code system definition, all the codes SHALL be unique",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="f:ValueSet/f:compose">
		<xsl:choose>
			<xsl:when test="exists(f:include) or exists(f:import)">
				<xsl:if test="$includeSuccessfulTests='1'">
					<xsl:text>{"status":1,</xsl:text>
					<xsl:text>"context":"f:ValueSet/f:compose",</xsl:text>
					<xsl:text>"assert":"exists(f:include) or exists(f:import)",</xsl:text>
					<xsl:text>"text":"vsd-1: A value set composition SHALL have an include or an import",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="not(exists(f:concept)) or not(exists(f:filter))">
				<xsl:if test="$includeSuccessfulTests='1'">
					...
					<xsl:text>"text":"vsd-11: Cannot have both concept and filter",</xsl:text>
					<xsl:text>"location":"</xsl:text><xsl:apply-templates select="." mode="currentXPathWithPos"/><xsl:text>"},</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
