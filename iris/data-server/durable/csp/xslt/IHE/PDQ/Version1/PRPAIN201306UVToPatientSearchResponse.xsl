...
<xsl:choose>
    <xsl:when test="contains(@value,'mailto:')">
    <Email><xsl:value-of select="substring-after(@value,'mailto:')"/></Email>
    </xsl:when>
    <xsl:otherwise>
...
<Names>
<xsl:for-each select="$personRoot/hl7:name">
<!-- add Pseudonym -->
<xsl:if test="@use='A'">
<PersonName>
...
<xsl:variable name="familyAtBirth" select="$personRoot/hl7:name/hl7:family[@qualifier='BR']/text()"/>
<xsl:variable name="givenAtBirth" select="$personRoot/hl7:name/hl7:given[@qualifier='BR']/text()"/>
<xsl:variable name="prefixAtBirth" select="$personRoot/hl7:name/hl7:prefix[@qualifier='BR']/text()"/>
...
<xsl:for-each select='$personRoot/hl7:personalRelationship'> 
<xsl:variable name="relationshipType" select="hl7:code/@code"/>
<xsl:choose>
<xsl:when test="$relationshipType = 'MTH' or ($relationshipType='NMTH') or ($relationshipType='NMTHF') or ($relationshipType='ADOPTM') or ($relationshipType='STPMTH')">
<MothersName>
<xsl:apply-templates select="hl7:relationshipHolder1/hl7:name"/>
</MothersName>
</xsl:when> 
<xsl:when test="($relationshipType = 'FTH') or ($relationshipType='ADOPTF') or ($relationshipType='NFTH') or ($relationshipType='NFTHF') or ($relationshipType='STPFTH')">
<FathersName>
<xsl:apply-templates select="hl7:relationshipHolder1/hl7:name"/>
</FathersName>
</xsl:when> 
...
<xsl:template name="name" match="hl7:name">
