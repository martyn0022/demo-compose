...
<rs:RegistryErrorList highestSeverity="{$highest}">
 <xsl:apply-templates select="//rs:RegistryError"/>
  ...
  <rs:RegistryError codeContext="{Description/text()}" errorCode="{Code/text()}" severity="{$severity}">
	<xsl:choose>
	<xsl:when test="Location/text() != ''">
	<xsl:attribute name="location"><xsl:value-of select="Location/text()"/></xsl:attribute>
	</xsl:when>
	<xsl:otherwise>
	<xsl:attribute name="location"><xsl:value-of select="$homeCommunityID"/></xsl:attribute>
	</xsl:otherwise>
	</xsl:choose>
  </rs:RegistryError> 
  </xsl:for-each>
...
<xsl:if test="$homeCommunityID !=''">
<xsl:attribute name="location"><xsl:value-of select="$homeCommunityID"/></xsl:attribute>
</xsl:if>
<xsl:if test="$homeCommunity ='' and xdsb:Location!=''">
...
<xsl:choose>
<xsl:when test="not(./xdsb:HomeCommunityId/text())">
<HomeCommunityId>urn:oid:<xsl:value-of select="$homeCommunityID"/></HomeCommunityId>
...
<xsl:otherwise>
<xsl:attribute name="location">urn:oid:<xsl:value-of select="$homeCommunityID"/></xsl:attribute>
</xsl:otherwise>
</xsl:choose>
<xsl:attribute name="codeContext"><xsl:value-of select="@codeContext"/></xsl:attribute>
