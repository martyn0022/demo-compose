...
<!-- 
XCA RetrieveDocumentSet-b Audit Message (ITI-39) 
-->
...
<xsl:variable name="eventType" select="'ITI-39,IHE Transactions,Cross Gateway Retrieve'"/>
<xsl:variable name="status"    select="/Root/Response//rs:RegistryResponse/@status"/>
<xsl:variable name="isSource"  select="$actor='XCARespondingGateway'"/>

<xsl:template match="/Root">
<Aggregation>

<xsl:choose>
<xsl:when test="$actor='XCAInitiatingGateway'">
...
<xsl:if test="not($actor='XCAInitiatingGateway')">
	<EnsembleSessionId>
...
<xsl:if test="$actor='XCAInitiatingGateway'">
	<EnsembleSessionId>
		<xsl:value-of select="$session"/>
	</EnsembleSessionId>
</xsl:if>
