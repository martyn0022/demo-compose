...
<!--
PIXv2 Query Audit Message (ITI-9)
-->
...
<xsl:variable name="eventType" select="'ITI-9,IHE Transactions,PIX Query'"/>
<xsl:variable name="status"><xsl:value-of select="$v2Status"/></xsl:variable>
<xsl:variable name="isSource"  select="$actor='PIXv2Consumer'"/>
